...
- Fonts and typography: serif display treatment is preserved for Today, Day label, and vocabulary words; sans-serif UI labels are readable. The updated timeline uses a narrower time rail so real words, audio, and hide controls fit more comfortably on mobile.
...
- Copy and content: app uses real source fields for word, explanation, synonyms, PDF titles, and page counts. Pronunciation is enriched from dictionary audio URLs when available; IPA and detailed dictionary prose are still not invented.
...
- Replaced browser SpeechSynthesis with cached `dictionaryapi.dev` audio lookup and no synthetic-voice fallback.
- Converted Today section headings into real navigation buttons for Vocabulary, Review, and Practice.
- Tightened the mobile timeline, focus card, and bottom navigation safe area to reduce tap overlap and clipping.
